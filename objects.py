    ...
    def set_image(self):
        if not self.is_attacking:
            ...
            if not (self.was_right or self.was_left):
                ...
                if self.was_right:
                    self.image = ani.punchRight[min(self.attack_count // 5, 11)]
                else:   # else, self.was_left is true
                    self.image = ani.punchLeft[min(self.attack_count // 5, 11)]
...
class TemporaryCollectible(pg.sprite.Sprite):
    def __init__(self, item_num, area=(15, 15)):
        super().__init__()
        ...
        if item_num == 0:
            ...
            self.image.fill(cons.BLUE)
        else:
            self.image.fill(cons.SILVER)
